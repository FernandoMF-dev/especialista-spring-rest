...
public interface StateControllerDocumentation {
	...
	ResponseEntity<StateDto> insert(@ApiParam(name = "body", value = "State data to create") StateDto dto);
	...
	@ApiResponse(responseCode = "404", description = "State not found", content = @Content(schema = @Schema(implementation = ApiErrorResponse.class)))
	ResponseEntity<StateDto> update(@ApiParam(value = "ID of a state to update", example = "1") Long id,
									@ApiParam(name = "body", value = "State data to update") StateDto dto);

	@ApiOperation("Delete a state by its ID")
	@ApiResponse(responseCode = "204", description = "State deleted")
	@ApiResponse(responseCode = "404", description = "State not found", content = @Content(schema = @Schema(implementation = ApiErrorResponse.class)))
	ResponseEntity<Void> delete(@ApiParam(value = "ID of a state to delete", example = "1") Long id);
}
